...
/// A wrapper for async-std's `JoinHandle`.
#[pin_project]
pub struct AsyncStdJoinHandle<T>(#[pin] async_std::task::JoinHandle<Result<T, Canceled>>);

impl<T> Future for AsyncStdJoinHandle<T> {
    type Output = Result<Result<T, Canceled>, Infallible>;
...
impl<T> RuntimeJoinHandle for AsyncStdJoinHandle<T>
where
    T: Send + 'static,
{
    type Output = T;
...
///
/// This future is created by [`spawn`] function and avaiable when `"async-std"` feature is enabled.
...
where
    ...
    T: Send + 'static,
{
    type Output = JoinHandle<AsyncStdJoinHandle<T>>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        let future = self.0.take().expect("already spawned");
        let inner = AsyncStdJoinHandle(async_std::task::spawn(install(future, cx)));
...
///
/// This function is available when `"async-std"` feature is enabled.
///
